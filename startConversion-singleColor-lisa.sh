...
#	# Check for DCM now
# DIRINPUT #	sh ./sh/prerequites-dcm.sh
#	if [ $? -ne 0 ]; then
#		echo "Missing prerequisites, install them (check output)."
#		exit 1
#	fi
...
if [ ! -r $BORDER_DIR/$BORDER_FILE ]; then
	...
	echo -n " ./py/computation/step_generatebordermtx.py -x $BORDER_X -y $BORDER_Y -z $BORDER_Z -o $BORDER_DIR"
...
if [ $OPENCL -eq 1 ]; then
   ...
   echo ""
	...
	echo -n "Computing output binary chains... "
	CHAINCURR=$COLOR_SEL
	echo -n "## params  $BORDER_DIR/$BORDER_FILE -x $BORDER_X -y $BORDER_Y\
        -z $BORDER_Z -i $DIRINPUT -c $COLORS -d $CHAINCURR -q $BESTFILE\
        -o $COMPUTATION_DIR_BIN "
	#$PYBIN ./py/computation/step_calcchains_serial_tobinary_filter_proc.py -r -b $BORDER_DIR/$BORDER_FILE -x $BORDER_X -y $BORDER_Y -z $BORDER_Z -i $TMPIMGDIRECTORY -c $COLORS -d $CHAINCURR -q $BESTFILE -o $COMPUTATION_DIR_BIN 
