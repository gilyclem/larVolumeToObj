...
def removeDoubleVertexesAndFaces(vertexes, faces, boxsize=None, index_base=0,
    ...
    if use_dict_algorithm:
        ...
        if boxsize is None:
            ...
            # Removing all vertices not linked with a face
            toRemove = np.ones(len(new_vertexes), dtype = np.bool)
            ...
            # Reindexing faces
            ind_vertices = np.arange(len(new_vertexes), dtype = np.int)
            
            prevV = 0
            for i in xrange(0, len(new_vertexes)):
                if toRemove[i] == False:
                    ind_vertices[i] = prevV
                    prevV += 1
                    
            
            new_faces = reindexVertexesInFaces(new_faces, ind_vertices,
                                               index_base=index_base)
            
            # Effectively removing vertices
            new_vertexes2 = []
            for i in xrange(0, len(toRemove)):
                if toRemove[i] == False:
                    new_vertexes2.append(new_vertexes[i])
            new_vertexes = new_vertexes2
