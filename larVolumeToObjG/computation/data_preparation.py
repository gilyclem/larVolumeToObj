...
import numpy as np
import io3dG
import io3dG.datareader
import io3dG.datawriter
import sed3

import scipy.ndimage as ndimage

def preparedata(inputfile, outputfile='prepared.pklz', crop=None, threshold=None,
                visualization=False, zero_border=0, label=2, morphology=False, savePng=False):
    """
    ...
    :return:
    """ 
    datap = io3dG.datareader.read(inputfile, dataplus_format=True)
    print "data acquired"
    """
    if crop is not None:
    ...
    """
    #Morphology operations: binary closing and binary opening
    if morphology:
    	struct_elem_sz = 2
    	struct = np.ones((struct_elem_sz,struct_elem_sz,struct_elem_sz))
	
    	datap['segmentation']=ndimage.binary_closing(datap['segmentation'],struct).astype(np.uint8)
    	datap['segmentation']=ndimage.binary_opening(datap['segmentation'],struct).astype(np.uint8)
	...
	if os.path.exists('datap'):
        	shutil.rmtree('datap')
    	pklzToSmoothObj.mkdir_p('datap')
        for i in range(len(datap['segmentation'])):
                img = [datap['segmentation'][i][j]*255 for j in range(len(datap['segmentation'][i]))]
		scipy.misc.toimage(img).save('datap/out_'+str(i)+'.png')
    
    io3dG.datawriter.write(datap, outputfile)	
