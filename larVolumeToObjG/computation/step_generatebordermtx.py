...
def computeNonOrientedBordo3(nx, ny, nz):
    from larlib import boundary
    V, [VV, EV, FV, CV] = getBases(nx, ny, nz)
    boundaryMat = boundary(CV,FV)
    return boundaryMat
...
def getBordo3Path(nx, ny, nz, DIR_OUT, oriented=False):
    """
    Function try read bordo3 from file. If it fail. Matrix is computed
    """
	...
	if oriented:
        	bordo3 = computeOrientedBordo3(nx, ny, nz)
	else:
		bordo3 = computeNonOrientedBordo3(nx,ny,nz)
        writeBordo3(bordo3, fileName)
    return fileName
