...
def convert(
    ...
    nx, ny, nz = bordersize
    brodo3path = gbmatrix.getBordo3Path(nx, ny, nz, borderdir)
...
def makeSmooth(
    ...
    V, F = makeCleaningAndSmoothing(
        V, F,
        os.path.join(outputdir, outputfile),smoothing)
    print "After"
    print "Number of vertexes: %i    Number of faces %i" % (len(V), len(F))
# write to ints
# fill empty vertexes
...
def makeCleaningAndSmoothing(V, F, outputfile=None, smoothing=True):
